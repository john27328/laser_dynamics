...
//      ADD_VAR(L_eff, L + (n - 1) * l, "cm")
    ...
    ADD_CONST(lKr,40, "mm", "m", 1000, 1)
      ADD_VAR(l, lKr/NKr, "m") //разбил кристалл
    ADD_CONST(P, 20, "W полная мощность, дожигание", "W", 1, 1)
      ADD_VAR(P_pump[NKr], P, "объявляю мощность")
    ADD_CONST(T_doj, 1000, "% дожигание", "1", 100, 1)
    ...
    ADD_CONST(h, 6.62e-34, "Дж*с", "Дж*с", 1, 0)
      ...
      ADD_VAR(rho_full, rho_bad - c / L * (log(1 - T1) + log(1 - T2))/ 2, "c-1" )
      ADD_VAR(rho, rho_full/NKr, "c-1")


//нормировка
    //ADD_VAR(R0n,R0 / N0, "c-1")
      ADD_VAR(R0n[NKr], 0, "c-1") //объявляю

    ADD_VAR(K_ap_n, K_ap * N0, "c-1")
    ADD_VAR(P_pog_pump, P * (1 - exp(-K_sigma * l)), "W")
    ADD_VAR(q_pump, P_pog_pump * L * lambda_pump / h / pow(c,2), "1")
