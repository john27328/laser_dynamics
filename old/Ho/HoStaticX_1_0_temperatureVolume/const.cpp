...
//      ADD_VAR(L_eff, L + (n - 1) * l, "cm")
    ...
    //ADD_CONST(q1, 0, "минимальный квантовый выход", )
    ADD_CONST(Nrasb, 10, "число разбиений кристалла", "1", 1, 1)
    ...
    ADD_CONST(sigma_pump_pr, 100, "% поправочный коэффициент", "1", 100, 1)
    //ADD_CONST(K_sigma, 0.96, "cm-1", "m-1", 1e-2, 1)
    ADD_CONST(d, 0.55, "mm диаметр накачки", "m", 1000, 1)
    ADD_CONST(rho_bad, 0.035, "c-1", "c-1", 1, 1)
    ADD_CONST(L, 12, "cm", "m", 100, 1)
    ADD_CONST(T1, 0.3, "% глухое зеркало", "1", 100, 1)
    ADD_CONST(T2, 35, "% выходное зеркало", "1", 100, 1)
    ADD_CONST(T_doj, 100, "% дожигание", "1", 100, 1)
    ADD_CONST(l_kr,40, "mm", "m", 1000, 1)
      ADD_VAR(l, l_kr/Nrasb, "m")
    ADD_CONST(P, 20, "W полная мощность", "W", 1, 1)
    ...
    ADD_CONST(K, 1.38e-23, "Дж/К", "1", 1, 0)
    ADD_CONST(TRoom, 300, "K", "K", 1, 1)
